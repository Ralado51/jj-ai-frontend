    ...
    <main className="relative grid min-h-screen overflow-hidden bg-background px-5 py-10 lg:grid-cols-2 lg:px-10">
      ...
      <section className="relative hidden flex-col justify-between rounded-3xl border bg-surface/70 p-10 shadow-glow backdrop-blur lg:flex">
        <div className="flex items-center gap-3">
          <div className="grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-primary to-secondary text-white">
            <Bot size={22} />
          </div>
          <div>
            <p className="font-[var(--font-manrope)] text-lg font-bold">JJ AI Platform</p>
            <p className="text-xs text-muted">AI Premium Workspace</p>
          </div>
        </div>

        <div className="max-w-xl">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border bg-primary/10 px-3 py-1 text-xs font-medium text-secondary">
            <Sparkles size={14} />
            Inteligência aplicada
          </div>
          <h1 className="font-[var(--font-manrope)] text-5xl font-bold leading-tight tracking-tight">
            Operação conectada para criar, automatizar e escalar.
          </h1>
          <p className="mt-6 max-w-lg text-base leading-7 text-muted">
            Centralize projetos, agentes, conhecimento e workflows em uma única plataforma segura.
          </p>
        </div>
      ...
      <section className="relative grid place-items-center py-8 lg:py-0">
        <div className="w-full max-w-md rounded-3xl border bg-surface/90 p-7 shadow-glow backdrop-blur md:p-9">
          <div className="mb-8 lg:hidden">
            <div className="mb-4 grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-primary to-secondary text-white">
              <Bot size={22} />
            </div>
            <p className="font-[var(--font-manrope)] text-xl font-bold">JJ AI Platform</p>
          </div>

          <p className="text-sm font-medium text-secondary">Acesso seguro</p>
          <h2 className="mt-2 font-[var(--font-manrope)] text-3xl font-bold tracking-tight">Entrar na plataforma</h2>
          <p className="mt-3 text-sm leading-6 text-muted">Use sua conta cadastrada na API da JJ AI Platform.</p>
          ...
          <form className="mt-8 space-y-5" onSubmit={handleSubmit(onSubmit)}>
            ...
            <button
              ...
              className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-primary to-secondary font-semibold text-white transition hover:brightness-110 disabled:cursor-wait disabled:opacity-70"
            >
