  ...
  return <DashboardShell><section className="mx-auto max-w-[1500px] space-y-6">
    <header className="rounded-3xl border bg-surface p-6 shadow-glow md:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex gap-4"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-primary/15 text-secondary"><Workflow /></div><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-secondary">Execution Center</p><h1 className="mt-2 text-3xl font-bold">Execuções de workflows</h1><p className="mt-2 text-sm text-muted">Acompanhe status, duração, etapas concluídas e conteúdo final.</p></div></div>
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_420px]">
      ...
      <aside className="h-fit rounded-2xl border bg-surface p-5 xl:sticky xl:top-24">
        {selected ? <div className="space-y-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-wide text-secondary">Execução</p><h2 className="mt-1 text-xl font-bold">{selected.workflow_name}</h2></div><StatusIcon status={selected.status} /></div><div className="grid grid-cols-2 gap-3 text-sm"><div className="rounded-xl bg-background p-3"><p className="text-xs text-muted">Status</p><p className="mt-1 font-semibold">{statusLabel(selected.status)}</p></div><div className="rounded-xl bg-background p-3"><p className="text-xs text-muted">Duração</p><p className="mt-1 font-semibold">{(selected.total_duration_ms / 1000).toFixed(2)} s</p></div><div className="rounded-xl bg-background p-3"><p className="text-xs text-muted">Etapas</p><p className="mt-1 font-semibold">{selected.steps_completed}/{selected.steps_total}</p></div><div className="rounded-xl bg-background p-3"><p className="text-xs text-muted">Memória</p><p className="mt-1 font-semibold">{selected.use_memory ? "Ativa" : "Desativada"}</p></div></div><div><p className="text-xs font-semibold uppercase tracking-wide text-muted">Instrução</p><p className="mt-2 whitespace-pre-wrap rounded-xl bg-background p-3 text-sm leading-6">{selected.instruction}</p></div>{selected.error_message ? <div><p className="text-xs font-semibold uppercase tracking-wide text-red-300">Erro</p><p className="mt-2 rounded-xl bg-red-500/10 p-3 text-sm text-red-200">{selected.error_message}</p></div> : null}{selected.final_content ? <div><p className="text-xs font-semibold uppercase tracking-wide text-secondary">Conteúdo final</p><div className="mt-2 max-h-[420px] overflow-auto whitespace-pre-wrap rounded-xl bg-background p-3 text-sm leading-6">{selected.final_content}</div></div> : null}<p className="flex items-center gap-2 text-xs text-muted"><Clock3 size={14} /> Atualizada em {new Date(selected.updated_at).toLocaleString("pt-BR")}</p></div> : <p className="text-sm text-muted">Selecione uma execução para ver os detalhes.</p>}
      </aside>
    </div>
