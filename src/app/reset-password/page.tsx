      ...
      <section className="w-full max-w-md rounded-3xl border bg-surface p-8 shadow-glow">
        ...
        {success ? (
          ...
          <form className="mt-8 space-y-5" onSubmit={handleSubmit(onSubmit)}>
            <div><label htmlFor="password" className="mb-2 block text-sm font-medium">Nova senha</label><div className="flex items-center gap-3 rounded-xl border bg-elevated px-4 focus-within:border-primary"><LockKeyhole size={17} className="text-muted" /><input id="password" type="password" autoComplete="new-password" className="h-12 w-full bg-transparent text-sm outline-none" {...register("password")} /></div>{errors.password && <p className="mt-2 text-xs text-red-400">{errors.password.message}</p>}</div>
            <div><label htmlFor="confirmation" className="mb-2 block text-sm font-medium">Confirmar nova senha</label><div className="flex items-center gap-3 rounded-xl border bg-elevated px-4 focus-within:border-primary"><LockKeyhole size={17} className="text-muted" /><input id="confirmation" type="password" autoComplete="new-password" className="h-12 w-full bg-transparent text-sm outline-none" {...register("confirmation")} /></div>{errors.confirmation && <p className="mt-2 text-xs text-red-400">{errors.confirmation.message}</p>}</div>
