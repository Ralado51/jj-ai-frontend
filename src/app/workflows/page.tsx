...
export default function WorkflowsPage() {
  ...
  async function followExecution(executionId: string) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (selectedWorkflowId) {
        const queued = await runPersistedWorkflowAsync(selectedWorkflowId, {
          instruction: instruction.trim() || undefined,
      ...
      setError(
        axiosError.response?.data?.detail
          ?? (requestError instanceof Error ? requestError.message : "Não foi possível executar o workflow."),
      );
    } finally {
      setRunning(false);
    }
